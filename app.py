...
# ── 페이지 설정 ─────────────────────────────────────
st.set_page_config(page_title="Klook Travel Demand Radar", layout="wide", page_icon="klook-app.png")
...
# ── Supabase 연결 ──────────────────────────────────────
SUPABASE_URL = os.environ.get("SUPABASE_URL", "https://pdrynagoznsomxrrmlca.supabase.co")
SUPABASE_KEY = os.environ.get("SUPABASE_KEY", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBkcnluYWdvem5zb214cnJtbGNhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkwODYzMDIsImV4cCI6MjA5NDY2MjMwMn0.owcACYsMDb2m1s0kLYAdN3Y0OJhRRxD0q_neXXIzouU")
...
# ── 데이터 로더 (Supabase 우선, JSON fallback) ────────
def load_json(filename):
    """Supabase에서 로드, 없으면 로컬 JSON fallback."""
    if _supabase_available():
        try:
            doc_id = filename.replace(".json", "")
            resp = requests.get(
                f"{SUPABASE_URL}/rest/v1/app_settings?id=eq.{doc_id}&select=data",
...
def save_json(filename, data):
    """Supabase에 저장, 실패 시 로컬 JSON에도 저장."""
    # Supabase upsert
    if _supabase_available():
        try:
            doc_id = filename.replace(".json", "")
            body = {"id": doc_id, "data": data, "updated_at": "now()"}
            resp = requests.post(
...
def page_forecast():
    ...
    with col_top:
        ...
        st.dataframe(rank_display, use_container_width=True, height=430)

    with col_trend_map:
        st.markdown("**📊 상위 국가 트렌드 히트맵**")
        if not trend_api_df.empty:
            top10 = 국가순위[:10]
            t5_df = trend_api_df[trend_api_df["국가"].isin(top10)].copy()
            if not t5_df.empty:
                ...
                heat_pivot = heat_pivot.reindex(index=top10)
